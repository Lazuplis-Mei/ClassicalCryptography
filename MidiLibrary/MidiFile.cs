...
/// </summary>
public sealed partial class MidiFile : ICloneable
{
    /// <summary>
    ...
    public double Tempo => MicroTempoToTempo(MicroTempo);
    ...
    public IEnumerable<KeyValuePair<int, double>> Tempos
    {
        get
        {
            foreach (var mt in MicroTempos)
                yield return new KeyValuePair<int, double>(mt.Key, MicroTempoToTempo(mt.Value));
        }
    }
    ...
    public IEnumerable<KeyValuePair<int, MidiTimeSignature>> TimeSignatures
    {
        ...
        {
            if (Tracks.Count == 0)
                return Enumerable.Empty<KeyValuePair<int, MidiTimeSignature>>();
    ...
    public IEnumerable<KeyValuePair<int, MidiKeySignature>> KeySignatures
    {
        ...
        {
            if (Tracks.Count == 0)
                return Array.Empty<KeyValuePair<int, MidiKeySignature>>();
    ...
    public int FirstDownBeat
    {
        get
        {
            ...
            foreach (var trk in Tracks)
            {
                int d = trk.FirstDownBeat;
                if (d != -1 && (result != -1 || result > d))
                    result = d;
            }
            return result;
        }
    ...
    {
        if (!TryReadChunk(stream, out KeyValuePair<string, byte[]> chunk) || chunk.Key != "MThd" || chunk.Value.Length < 6)
    ...
    public MidiFile AdjustTempo(double tempo) => AdjustTempo(TempoToMicroTempo(tempo));
    ...
    public MidiFile GetRange(int start, int length, bool copyTimingAndPatchInfo = false, bool eliminateEmptyTracks = true)
    {
        ...
        foreach (var trk in Tracks)
        {
            ...
            if (!eliminateEmptyTracks || t.Events.Count > 0)
                result.Tracks.Add(t);
        }
        return result;
    }
    ...
    private static bool TryReadChunk(Stream stream, out KeyValuePair<string, byte[]> chunk)
    {
        chunk = default;
        var buf = new byte[4];
