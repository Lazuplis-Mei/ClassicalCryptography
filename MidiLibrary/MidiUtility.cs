...
/// </summary>
internal static partial class MidiUtility
{
    private const string _Notes = "C C#D D#E F F#G G#A A#B ";
    private const string _Notes2 = "C DbD EbE F GbG AbA BbB ";
    ...
    {
        noteId -= 12;//这可能引起争议(但我使用和MMA相同的配置)
    ...
    public static double MicroTempoToTempo(int microTempo) => 60000000 / ((double)microTempo);
    ...
    public static int TempoToMicroTempo(double tempo) => (int)(500000 * (120d / tempo));
    ...
    public static ushort Swap(ushort x) => (ushort)((ushort)((x & 0xff) << 8) | ((x >> 8) & 0xff));
    ...
    public static uint Swap(uint x) => ((x & 0x000000ff) << 24) + ((x & 0x0000ff00) << 8) + ((x & 0x00ff0000) >> 8) + ((x & 0xff000000) >> 24);
