...
sealed partial class MidiFile : ICloneable
{
    ...
    public double Tempo
    {
        get
        {
            return MicroTempoToTempo(MicroTempo);
        }
    }
    /// <summary>
    ...
    public IEnumerable<KeyValuePair<int, MidiTimeSignature>> TimeSignatures
    {
        ...
        {
            if (0 == Tracks.Count)
                return Array.Empty<KeyValuePair<int, MidiTimeSignature>>();
    ...
    public MidiFile AdjustTempo(double tempo)
        => AdjustTempo(TempoToMicroTempo(tempo));
    /// <summary>
    /// Adjusts the tempo of a MIDI file
    /// </summary>
    ...
    public static MidiFile ReadFrom(Stream stream)
    {
        ...
        if (!_TryReadChunk(stream, out KeyValuePair<string, byte[]> chunk) || "MThd" != chunk.Key || 6 > chunk.Value.Length)
